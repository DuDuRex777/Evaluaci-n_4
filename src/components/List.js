...
function Lista({ jugadores, eliminarJugador, editarJugador }) {
  ...
  return (
    <div className="mt-8">
      <h2 className="text-2xl mb-4 text-amber-800">Lista de Jugadores</h2>
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
        {jugadores.map((jugador) => (
          <div key={jugador.id} className="bg-orange-900 p-4 rounded-lg shadow-md relative">
            <div className="text-white">
              ...
              <p className="text-gray-100 mb-2">Número: {jugador.numero}</p>
              <p className="text-gray-100 mb-2">Fecha Nacimiento: {jugador.fecha_nacimiento}</p>
      ...
      <div className="mt-8">
        <h3 className="text-xl text-amber-800">Años de experiencia en promedio de los jugadores: {calcularEdadPromedio()} años</h3>
      </div>
    </div>
  );
}
